...
from .datatypes.resource_instances import StaticTile, STATIC_STORE, APIResourceInstanceInfo
...
class ResourceAPIResourceWrapper(ResourceWrapper, proxy=True):
    """ResourcesAPI wrapper for all well-known resources.
    ...
    @classmethod
    def all(cls, related_prefetch: Callable[[str], "ResourceWrapper"] | None=None, limit: int | None=None, lazy: bool=False) -> list["ResourceWrapper"]:
        """Get all resources of this type."""
        resources = STATIC_STORE.load_all(cls._wkrm.graphid, limit=limit)
        return [
    ...
    def get_fields(cls, include_root=False):
        ...
        def _fill_fields(pseudo_node):
            ...
            # FIXME: why does this break instantiation? print("get child type2", pseudo_node.get_child_types())
    ...
    def _get_allowed_tiles(
        ...
        if resourceinstance := kwargs.get("resourceinstance", []):
            resourceid = resourceinstance.resourceinstanceid
        ...
        tiles = STATIC_STORE.get_tiles(resourceid, *(node.nodeid for node in nodes))
    ...
    def where(cls, cross_record=None, lazy=False, case_i=False, **kwargs):
        ...
        for resource in STATIC_STORE.search_by_nodeids(
            *(
                (cls._node_objects_by_alias().get(key).nodeid, value)
                for key, value in kwargs.items()
            )
        ):
            yield cls.from_static_resource(resource, cross_record=cross_record, lazy=lazy)

    @classmethod
    def from_static_resource(cls, resource: APIResourceInstanceInfo, cross_record: dict[str, Any] | None=None, related_prefetch: Callable[[str], ResourceWrapper] | None=None, lazy: bool | None=False) -> "ResourceAPIResourceWrapper":
