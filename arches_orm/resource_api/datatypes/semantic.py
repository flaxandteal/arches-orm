...
from arches_orm.resource_api.datatypes.resource_instances import APIResourceInstanceInfo
...
def get_child_values(svm, target_key: str | None = None, parent = None, child_keys = None, child_nodes = None, tile = None, node = None):
    ...
    if not parent:
        return {} if target_key is None else None
    for key in (child_keys if target_key is None else [target_key]):
...
def semantic(
    tile,
    node,
    value: uuid.UUID | str | ResourceInstanceViewModel | APIResourceInstanceInfo | None,
    parent,
    parent_cls,
    child_nodes,
    datatype,
